...
public class InputHandler {
	...
	private void handleTransition(SharedPreferences sharedPreferences, Transition transition) {
		if(TransitionState.PENDING.equals(transition.getTransitionState())){
			if(transition instanceof NewBookingTransition) {
				...
				removeCancelation(transition, sharedPreferences);
				System.out.println("The cancel request has been removed \n");
			}
		} else {
	...
	private void removeCancelation(Transition transition, SharedPreferences sharedPreferences) {
		sharedPreferences.remove("T" + transition.getTransitionId());
...
//		}
	...
	private void printBookings() {
		...
		for (String key : keySet) {
			...
			Transition deserialized = (Transition) deserialize(all.get(key));
			String type = "";
			if(deserialized instanceof CancelBookingTransition){
				type = "CB";
			} else {
				type = "NB";
			}
			System.out.println(i + ". " + type + key + " " +
			formater.format(deserialized.getBooking().getTravelDate()) + " from " + deserialized.getBooking().getFrom() + 
			" " + deserialized.getTransitionState());
			i++;
		}
